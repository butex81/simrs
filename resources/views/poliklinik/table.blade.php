...
<div class="sbox">
	...
	<h3><i class="fa fa-table"></i> <?php echo $pageTitle ;?> <small>{{ $pageNote }}</small></h3>
		...
		<div class="sbox-tools" >
			<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Clear Search" onclick="reloadData('#{{ $pageModule }}','poliklinik/data?search=')"><i class="fa fa-trash-o"></i> Clear Search </a>
			<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Reload Data" onclick="reloadData('#{{ $pageModule }}','poliklinik/data?return={{ $return }}')"><i class="fa fa-refresh"></i></a>
	...
	@include( $pageModule.'/toolbar')
	 ...
	 <?php echo Form::open(array('url'=>'poliklinik/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->id }}">
					...
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->id }}" data-url="{{ url('poliklinik/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif			
				 ...
				 <td data-values="action" data-key="<?php echo $row->id ;?>" width="100" class="text-right">
					{!! AjaxHelpers::buttonAction('poliklinik',$access,$id ,$setting) !!}
